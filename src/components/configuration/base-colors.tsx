...
export function BaseColors({ config, theme }: BaseColorsProps) {
  ...
  return (
    <ConfigSection id={baseColorsId} title="Base colors">
      ...
      />
      <ColorPicker
        hexColor={config[theme].baseColor["default-100"]} // <--- Use "default-100"
        label="Default 100"
        type="default-100" // <--- Use "default-100"
        onChange={(hexColor) => setCssColor("default-100", hexColor, theme)} // <--- Use "default-100"
        onClose={(hexColor) =>
          setBaseColor({ "default-100": hexColor }, theme) // <--- Use "default-100"
        }
      />
    </ConfigSection>
  );
